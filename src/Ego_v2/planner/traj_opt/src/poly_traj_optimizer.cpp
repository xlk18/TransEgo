...
namespace ego_planner
{
  ...
  void PolyTrajOptimizer::addPVAJGradCost2CT(EIGENVEC &gdT, Eigen::VectorXd &costs, const int &K)
  {
    ...
    for (int i = 0; i < N; ++i)
    {
      ...
      for (int j = 0; j <= K; ++j)
      {
        ...
        // dynamic obstacles
        if (wei_dyn_obs_ > 0.0 && dynObstacleGradCostP(t + step * j, pos, gradp, costp))
        {
          gradViolaPc = beta0 * gradp.transpose();
          gradViolaPt = alpha * gradp.transpose() * vel;
          jerkOpt_.get_gdC().block<6, 3>(i * 6, 0) += wei_dyn_obs_ * omg * step * gradViolaPc;
          gdT(i) += wei_dyn_obs_ * omg * (costp / K + step * gradViolaPt);
          costs(0) += wei_dyn_obs_ * omg * step * costp; // adding to obstacle costs
        }
  ...
  bool PolyTrajOptimizer::dynObstacleGradCostP(const double t, const Eigen::Vector3d &p, Eigen::Vector3d &gradp, double &costp)
  {
    if (!pred_trajs_ || pred_trajs_->markers.empty())
      return false;
    ...
    double R_safe = 0.5; // Drone radius + clearance
    double k_exp = 0.5;  // Expansion per second

    for (const auto &marker : pred_trajs_->markers)
    {
      if (marker.points.empty())
        continue;

      // Calculate index corresponding to accurate absolute time
      double dt = 0.1; // 10Hz prediction configured
      double t_pred_start = marker.header.stamp.toSec();
      double t_shifted = (t_now_ + t) - t_pred_start; // Global absolute time mapping

      int idx = std::round(t_shifted / dt);

      if (idx < 0)
        idx = 0;
      if (idx >= (int)marker.points.size())
        idx = marker.points.size() - 1;

      Eigen::Vector3d p_obs(marker.points[idx].x, marker.points[idx].y, marker.points[idx].z);

      // 利用传入颜色通道读取 MOT 预估的目标轨迹方差（取 x, y, z 中最大的方差以增强安全性）
      double std_dev = 0.0;
      if (idx < marker.colors.size()) {
          std_dev = std::max({marker.colors[idx].r, marker.colors[idx].g, marker.colors[idx].b});
      }

      // 新避障半径 = 基础无人机半径间隙 + 基于神经网路预估的标准差膨胀 + 时间拉大带来的额外发散容忍
      double R_eff = R_safe + std_dev + k_exp * std::max(0.0, t_shifted);
      
      Eigen::Vector3d dist_vec = p - p_obs;
      double dist = dist_vec.norm();

      if (dist < R_eff)
      {
        double dist_err = R_eff - dist;
        ret = true;
        costp += pow(dist_err, 3);
        if (dist > 1e-5) // Prevent div by zero
        {
          Eigen::Vector3d dist_grad = dist_vec / dist;
          gradp += -3.0 * dist_err * dist_err * dist_grad;
        }
      }
    }
    return ret;
  ...
  void PolyTrajOptimizer::setParam(ros::NodeHandle &nh)
  {
    ...
    nh.param("optimization/weight_dynamic_obstacle", wei_dyn_obs_, 1000.0);

    // Subscribe to prediction trajectories
    pred_sub_ = nh.subscribe("/traj_prediction/predicted_trajectories", 1, &PolyTrajOptimizer::predTrajCallback, this);
  }
